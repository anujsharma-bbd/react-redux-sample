...
module.exports = React.createClass({
  ...
  componentWillReceiveProps: function componentWillReceiveProps(newProps) {
    this.setState({ menu: this.makeMenu(newProps.children) }, function () {
      if (newProps.children && newProps.children.length && (this.isOpen || document.activeElement === this.refs.input)) {
        if (!(this.state.menu.children && this.state.menu.children.length)) {
          return;
        }
        this.setState({
          isOpen: true
  ...
  */
  ...
  clampIndex: function clampIndex(index) {
    if (index < 0) {
      return (this.props.children ? this.props.children.length - 1 : 0);
    } else if (index >= this.props.children ? this.props.children.length : 0) {
      return 0;
    }
    return index;
  },
  ...
  focusOptionAtIndex: function focusOptionAtIndex(index) {
    ...
    this.showList();
    var length = this.props.children ? this.props.children.length : 0;
  ...
  render: function render() {
    ...
    return div({ className: this.getClassName() }, this.props.value, this.state.inputValue, input({
      ...
      onPaste: (event, obj) => { this.props.onPaste(event, obj); return false; },
